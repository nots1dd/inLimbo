...
struct Sound {
    ...
    void initializeBuffers() {
        ...
        auto ringBufferSamples = static_cast<size_t>(
            RING_BUFFER_SECONDS * sampleRate * channels
        );
        ring = std::make_unique<RingBuffer<float>>(ringBufferSamples);
        
        // Decode buffer: DECODE_BUFFER_SECONDS of audio or MIN_DECODE_BUFFER_FRAMES, whichever is larger
        // This ensures we can handle large codec frames
        size_t decodeBufferFrames = std::max(
            static_cast<size_t>(DECODE_BUFFER_SECONDS * sampleRate),
