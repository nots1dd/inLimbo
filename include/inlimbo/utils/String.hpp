...
inline INLIMBO_API_CPP auto iequals_fast(const std::string& a, const std::string& b) noexcept -> bool {
    if (a.size() != b.size())
        return false;
...
inline INLIMBO_API_CPP auto iequals_fast_prelowered(const std::string& lowerA, const std::string& b) noexcept -> bool {
