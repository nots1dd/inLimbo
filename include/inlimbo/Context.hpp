...
{
  args.add<std::string>(
    "General", "song", 's', "Play song name", std::nullopt, [](const std::string& s) -> bool
    { return !s.empty(); }, "song name cannot be empty, if not provided defaults to null!");

  args.add<float>(
    "General", "volume", 'v', "Initial playback volume (0-100)", 75.0f, [](float vol) -> bool
    { return vol >= 0.0f && vol <= 100.0f; }, "Volume must be between 0.0 and 100.0");

  args.add<std::string>(
    "Modify", "edit-metadata", 'e', "Edit metadata (title only for now) before playback",
    std::nullopt, [](const std::string& s) -> bool { return !s.empty(); },
    "Metadata edit requires a non-empty song name, will default to null");

  args.addFlag("Modify", "rebuild-library", 'r', "Force rebuild of the music library cache");

  args.addFlag("Query", "print-artists", 'A', "Print all artists and exit");

  args.addFlag("Query", "print-albums", 'L', "Print all albums and exit");

  args.addFlag("Query", "print-genre", 'G', "Print all genres and exit");

  args.addFlag("Query", "print-summary", 'S', "Print library summary and exit");

  args.addFlag("Query", "songs-paths", 'P', "Print all song paths and exit");

  args.add<std::string>(
    "Query", "songs-artist", 'a', "Print all songs by artist name", std::nullopt,
    [](const std::string& s) -> bool { return !s.empty(); }, "Artist name defaults to null.");

  args.add<std::string>(
    "Query", "songs-album", 'l', "Print all songs by album name", std::nullopt,
    [](const std::string& s) -> bool { return !s.empty(); }, "Album name defaults to null.");

  args.add<std::string>(
    "Query", "songs-genre", 'g', "Print all songs by genre name", std::nullopt,
    [](const std::string& s) -> bool { return !s.empty(); }, "Genre name defaults to null.");
}
...
struct AppContext
{
  ...
  AppContext(const std::string& program, const std::string& description)
      : cmdlineArgs(program, description), parser(utils::DEBUG_LOG_PARSE)
  {
  ...
  switch (ctx.printAction)
  {
    ...
    case PrintAction::SongsByArtist:
    {
      const std::string artistName =
        ctx.cmdlineArgs.getOptional<std::string>("print-songs-by-artist").value_or("");
    ...
    case PrintAction::SongsByAlbum:
    {
      const std::string albumName =
        ctx.cmdlineArgs.getOptional<std::string>("print-songs-by-album").value_or("");
    ...
    case PrintAction::SongsByGenre:
    {
      const std::string genreName =
        ctx.cmdlineArgs.getOptional<std::string>("print-songs-by-genre").value_or("");
...
inline auto initializeContext(int argc, char** argv) -> AppContext
{
  ...
  const float vol = ctx.cmdlineArgs.getOptional<float>("volume").value_or(75.0f);
  ...
  LOG_INFO("Configured directory: {}, song query: {}", ctx.musicDir, ctx.songName);
...
inline static void buildOrLoadLibrary(AppContext& ctx)
{
  ...
  {
    for (auto& [_, md] : ctx.parser.parseFromInode(inode, utils::DIRECTORY_FIELD))
    {
  ...
  LOG_INFO("Library rebuilt in {:.3f} ms", ctx.timer.elapsed_ms());
}
