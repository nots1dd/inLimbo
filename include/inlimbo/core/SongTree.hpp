...
struct Song
{
   ...
   */
  Song(unsigned int inode, Metadata  metadata) : inode(inode), metadata(std::move(metadata)) {}
...
public:
   ...
   */
  void display(const bool print_tree = false) const
  {
    RECORD_FUNC_TO_BACKTRACE("Dirsort");
    
    int                   totalArtists = 0, totalAlbums = 0, totalDiscs = 0, totalSongs = 0;
    std::set<Genre> uniqueGenres; // To count unique genres

    if (print_tree) std::cout << "\n=== Music Library Overview ===\n";
    if (print_tree) std::cout << "─────────────────────────────────────────────────────────────────────────────\n";
