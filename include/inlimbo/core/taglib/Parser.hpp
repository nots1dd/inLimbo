...
struct Metadata
{
  ...
  Artist artist = "<Unknown Artist>"; /**< The artist of the song */
  Album album = "Unknown Album"; /**< The album the song is part of */
  Genre genre = "Unknown Genre"; /**< The genre of the song */
  std::string comment = "No Comment"; /**< The comment associated with the song */
  std::string fileType = "NULL";
  Year year = 0; /**< The year of release */
  Track track = 0; /**< The track number */
  Disc discNumber = 0; /**< The disc number in a multi-disc set */
  Lyrics lyrics = "No Lyrics"; /**< The lyrics of the song */
...
auto TagLibParser::parseFile(const std::string& filePath, Metadata& metadata) -> bool {
  ...
  if (!file.tag()) {
    errMsg = "No tag information found in file: " + filePath;
    sendErrMsg(debugLogBoolStr, errMsg);
    if (debugLogBool) {
      LOG_ERROR(errMsg);
    }
  ...
  if (debugLogBool) {
    LOG_WARN("Track: {}", (tag->track() == 0 ? "(No Track, using fallback)" : std::to_string(tag->track())));
  ...
  if (debugLogBool) LOG_DEBUG("Assigned track number: ", metadata.track);
  ...
  if (audioProperties) {
    ...
    if (debugLogBool) {
      LOG_DEBUG("Audio properties found: ");
      LOG_DEBUG("-> Duration: {}secs", metadata.duration);
      LOG_DEBUG("-> Bitrate: {}kbps", metadata.bitrate);
    }
  } else {
    if (debugLogBool) {
      LOG_ERROR("No audio properties found!");
    }
  ...
  if (debugLogBool) {
    LOG_DEBUG("-> File Path: {}", metadata.filePath);
  }
  ...
  if (properties.contains("DISCNUMBER")) {
    ...
    if (debugLogBool) {
      LOG_DEBUG("-> Disc Number: {}", metadata.discNumber);
    }
  } else {
    metadata.discNumber = 0;
    if (debugLogBool) {
      LOG_WARN("-> Disc Number: (Not Available)");
    }
  }
  ...
  if (properties.contains("LYRICS")) {
    ...
    if (debugLogBool) {
      LOG_WARN("Lyrics: (Not Available)");
    }
  }

  // Populate additional properties if needed
  if (!properties.isEmpty()) {
    if (debugLogBool) LOG_INFO("Additional properties found!");
    ...
    if (stat(entry.path().c_str(), &fileStat) == 0) {
      if (fileStat.st_ino == inode) {
        ...
        if (parseFile(entry.path().string(), metadata)) {
          ...
          tempErrMsg = "Unable to parse metadata for file: " + entry.path().string();
          sendErrMsg(debugLogBoolStr, tempErrMsg);
        }
      }
    } else {
      tempErrMsg = "Unable to stat file: " + entry.path().string();
      sendErrMsg(debugLogBoolStr, tempErrMsg);
    }
  }
...
auto extractThumbnail(const std::string& audioFilePath, const std::string& outputImagePath) -> bool {
    ...
    if (extension == "mp3") {
        ...
        outputFile.write(recast<cstr>(pictureData.data()), pictureData.size());
        outputFile.close();
