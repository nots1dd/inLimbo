...
public:
  ...
  {
    auto& adder = groupAdder(group);
    adder(makeSpec(longName, shortName), description,
  ...
  [[nodiscard]] auto help() const -> std::string { return m_options.help(); }
...
private:
  ...
  static auto useColor() -> bool { return isatty(fileno(stderr)); }
