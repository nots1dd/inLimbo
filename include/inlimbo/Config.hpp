...
#define INLIMBO_VERSION INLIMBO_VERSION_ENCODE(INLIMBO_VERSION_MAJOR, INLIMBO_VERSION_MINOR, INLIMBO_VERSION_PATCH)
...
constexpr auto scast(A&& a) noexcept -> T { return static_cast<T>(std::forward<A>(a)); }

template <typename T, typename A>
constexpr auto recast(A&& a) noexcept -> T { return reinterpret_cast<T>(std::forward<A>(a)); }

template <typename T, typename A>
constexpr auto dcast(A&& a) noexcept -> T { return dynamic_cast<T>(std::forward<A>(a)); }
...
#define PACK_BEGIN(name, align) _Pragma("pack(push, align)") struct ALIGNAS(align) name {
#define PACK_END() } _Pragma("pack(pop)")

#define ABI_NAMESPACE_BEGIN(ver) namespace ABI_v##ver {
#define ABI_NAMESPACE_END(ver) } using namespace ABI_v##ver;
#define ABI_LOCK(name, version) namespace name { static constexpr int ABI_MAJOR = version; static_assert(ABI_MAJOR == ABI_VERSION_MAJOR, "ABI mismatch"); }
...
#define VERIFY_OFFSET(type, field, expected) \
    static_assert(offsetof(type, field) == (expected), "Field offset mismatch in " #type "::" #field)

#define VERIFY_LAYOUT(type, size, align) \
    VERIFY_SIZE(type, size); VERIFY_ALIGN(type, align)

// ============================================================
// Optional ABI/Feature Macros
// ============================================================
#define CHECK_ABI_COMPAT(old, newtype) \
    static_assert(sizeof(old) == sizeof(newtype) && alignof(old) == alignof(newtype), "ABI layout mismatch")

#define SYMBOL_GUARD(sym) \
    extern "C" void sym() __attribute__((weak, alias("__abi_guard_" #sym)))
...
#define ABI_FEATURE_FLAG(name) \
    static constexpr const char* CONCAT(abi_feature_, name) = #name

#define ABI_NAMESPACE(ver) \
    namespace abi_##ver

#define ABI_EXPORT_STRUCT(name, ver) \
    ABI_NAMESPACE_BEGIN(ver) struct name; ABI_NAMESPACE_END(ver)
