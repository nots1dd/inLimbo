...
void DirectoryWalker::walkFd(int dirFd, const EntryCallback& cb) {
    ...
    while ((ent = readdir(dir)) != nullptr) {
        ...
        if (S_ISLNK(st.st_mode)) {
            ...
            //
            // Report: records symlink paths but doesnt recurse thru it and the caller (cb) will decide what to do.
            // this can be used to showcase ALL the symlinks and do something about it.
            //
            // Follow: (NOT RECOMMENDED!) This follows the symlink and recursively walks thru it if directory else callback is invoked.
            // there *shouldnt* be a need for this as this can cause infinite recursion and a whole lotta problems.
            ...
            if (m_symlinkPolicy == SymlinkPolicy::Follow) {
                ...
                if (S_ISDIR(target.st_mode)) {
                    int childFd = openat(dirFd, ent->d_name,
