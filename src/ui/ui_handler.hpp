...
private:
    ...
    // Main data structure
    std::map<std::string, std::map<std::string, std::map<unsigned int, std::map<unsigned int, Song>>>> library;
            ...
            for (const auto& album_pair : artist_pair.second) {
                for (const auto& disc_pair : album_pair.second) {
                    for (const auto& track_pair : disc_pair.second) {
                        ...
                        // Convert metadata fields to lower case for case-insensitive search (it doesnt work lmao)
                        ...
                        if (title_lower.find(query_lower) != std::string::npos ||
                            ...
                            std::stringstream ss;
                            ss << artist_pair.first << " - " << song.metadata.title 
                               << " (" << album_pair.first << ")";
                            current_song_names.push_back(ss.str());
                        }
                    }
