...
void MetadataOverlay::draw(const ui::Fonts& fonts, const Metadata& meta)
{
  ...
  DrawRectangleRounded(
    {box.x + 6, box.y + 6, box.width, box.height},
    0.08f, 12, {0, 0, 0, 120});
  ...
  DrawTextEx(fonts.bold, "Track Information",
             {box.x + 24, box.y + 20},
             22, 1, TEXT_MAIN);

  DrawLine(box.x + 24, box.y + 52,
           box.x + box.width - 24, box.y + 52,
           {60, 60, 60, 255});
  ...
  {
    DrawTextEx(fonts.regular, label,
               {labelX, y},
               15, 1, TEXT_DIM);

    DrawTextEx(fonts.regular, value.c_str(),
               {valueX, y},
               16, 1, TEXT_MAIN);
  ...
  ui::text::drawWrappedText(fonts.regular,
              meta.filePath.c_str(),
              pathRect,
              14, 1, TEXT_MAIN);
  ...
  DrawTextEx(fonts.regular, hint,
             {box.x + box.width - hw - 24, box.y + box.height - 24},
             14, 1, TEXT_DIM);
}
