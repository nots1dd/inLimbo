...
const std::string SALT =
  "random_salt_value"; // This should match the server's salt basically (found in config.toml)
...
bool receive_file(SSL* ssl, const std::string& filename)
{
  ...
  if (!checksum.empty() && received_checksum != checksum)
  {
    ...
    std::cout << "[DEBUG] Calculated checksum from received data: " << received_checksum
              << std::endl;
