...
private:
        ...
        if (event.is_character())
        {
          switch (event.character()[0])
          {
            ...
            case 'v':
              ...
              return true;
            case 'l':
              current_view = ViewMode::Playlist;
              return true;
            case '/':
  ...
  Element RenderMainInterface(float progress)
  {
    ...
    // Status indicators
    std::string status = std::string("  ") + (is_playing ? "<>" : "!!") + " " +
                         (is_shuffle ? "🔀" : "➡") + " " +
    ...
    auto left_pane = vbox({
                     ...
                     border |
                     color(GetCurrWinColor(focus_on_artists)); // No flex here for vertical space.
    ...
    return vbox({
             ...
             hbox({
               ...
               text(current_view == ViewMode::Library    ? " Library "
                    : current_view == ViewMode::Playlist ? " Playlist "
                                                         : " Search ") |
